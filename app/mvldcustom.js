    ...
    $pc.fn.extend({
        ...
        build: function() {
       	...
       	//create the structure for pikachoose
			this.wrap 		= $("<div class='pika-stage-custom'></div>").insertBefore(this.list);
			this.image 		= $("<img>").appendTo(this.wrap);
			this.imgNav 	= $("<div class='pika-imgnav-custom'></div>").insertAfter(this.image);
			this.imgPlay 	= $("<a></a>").appendTo(this.imgNav);
			this.counter 	= $("<span class='pika-counter'></span>").appendTo(this.imgNav);
			if(this.options.autoPlay){ this.imgPlay.addClass('pause'); }else{ this.imgPlay.addClass('play'); }
			this.imgPrev 	= $("<a class='up'></a>").insertAfter(this.imgPlay);
			this.imgNext 	= $("<a class='down'></a>").insertAfter(this.imgPrev);
			...
			this.textPrev 	= $("<a class='up'>"+this.options.text.up+"</a>").appendTo(this.textNav);
			this.textNext	= $("<a class='down'>"+this.options.text.down+"</a>").appendTo(this.textNav);
			this.list.addClass('pika-thumbs-custom');
        ...
        bindEvents: function() {
        	...
        	this.wrap.bind('mouseenter',{self:this},function(e){
        		e.data.self.imgNav.stop(true,true);
	     ...
	     doAnimation: function(n,data){
				...
				self.aniImg.hide();
	     		...
	     		switch(n){
					case 0:
						//fade out then in
						self.image.stop(true,true).fadeOut(this.options.animationSpeed,function(){
							self.image.attr('src',data.source).fadeIn(this.options.animationSpeed,function(){
					...
					case 1:
						...
						$.when(
							self.image.fadeOut(this.options.animationSpeed),
							self.aniImg.eq(0).fadeIn(this.options.animationSpeed)).done(function(){
							self.finishAnimating(data);
						});
					...
					case 3:
						self.aniDiv.show().children("div:lt(5)").hide().each(function(index){
							...
							$(this).css({opacity:0.1,"width":aWidth}).show().delay(delay).animate({opacity:1,"height":self.image.height()},this.options.animationSpeed,'linear',function(){
					...
					case 4:
						self.aniDiv.show().children().hide().each(function(index){
							...
							$(this).css({opacity:0.1,"height":"100%"}).show().animate({opacity:1,"width":aWidth},this.options.animationSpeed,'linear',function(){
								if(self.aniDiv.find(" div").index(this) == 4){
									self.finishAnimating(data);
								}
							});
					...
					case 5:
						self.aniDiv.show().children().show().each(function(index){
							...
							$(this).css({"height":aHeight,"width":aWidth,"opacity":.01}).delay(delay).animate({"opacity":1},this.options.animationSpeed,function(){
								if(self.aniDiv.find(" div").index(this) == 24){
									self.finishAnimating(data);
								}
							});
					...
					case 6:
						...
						self.aniDiv.css({width:0}).show().animate({width:self.image.width()},this.options.animationSpeed,function(){
							self.finishAnimating(data);
					...
					case 7:
						...
						self.aniImg.show();
						self.aniWrap.css({left:self.wrap.outerWidth()}).show().animate({left:"0px"},this.options.animationSpeed,function(){
							self.finishAnimating(data);
						});
						break;
				}
...
$(document).ready(function (){
					$(".navimg").mouseover(function(){
						...
						src = src.slice(0, 9) + 'b' + src.slice(10);
						$(this).attr("src", src);});
					$(".navimg").mouseout(function(){
						var src=$(this).attr("src");
						src = src.slice(0, 9) + 'g' + src.slice(10);
						$(this).attr("src", src);});
				});